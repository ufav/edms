...
export const DocumentTable: React.FC<DocumentTableProps> = observer(({
  ...
  const getFileTypeInfo = (fileType: string, fileName?: string) => {
    ...
    // PDF файлы
    if (type.includes('pdf') || lowerFileName.endsWith('.pdf')) {
    ...
    // CAD файлы - проверяем и MIME тип и расширение файла
  ...
  return (
    <Box sx={{
      ...
      {/* Единый TableContainer для заголовка и тела таблицы */}
      <TableContainer 
        component={Paper} 
        sx={{
          boxShadow: 2,
        ...
        <Table sx={{ 
          ...
          <TableBody>
            {documents.map((document) => (
              ...
              >
                {showSelectColumn && isDocumentSelectable(document.id) && (
                  <TableCell
                    data-sticky="left"
                    sx={{
                      width: '50px',
