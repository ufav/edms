...
const TransmittalDialog: React.FC<TransmittalDialogProps> = observer(({
  ...
  const handleToggleCcsStatus = async (revision: any) => {
    ...
    try {
      const newStatus = revision.ccs_status === 'open' ? 'closed' : 'open';
      const transmittalId = transmittalStore.selectedTransmittal?.id;

      if (!transmittalId) {
        if (onShowNotification) {
          onShowNotification(t('common.error', { defaultValue: 'Ошибка' }), 'error');
        }
        return;
      }

      await transmittalsApi.updateRevisionCcsStatus(transmittalId, revision.id, newStatus);
      ...
      if (onShowNotification) {
        onShowNotification(
          t('transmittals.ccs_status_updated', { defaultValue: `CCS статус изменен на ${newStatus}` }),
          'success'
        );
      }
    } catch (error: any) {
      console.error('Error updating CCS status:', error);
                          ...
                          >
                            ...
                            >
                              ...
                              {initialData?.direction === 'in' && ['Approved', 'Approved with Comments', 'Not Reviewed'].includes(revision.workflow_status) && (
                                ...
                                >
                                  {revision.ccs_status === 'closed' ? 'Closed' : 'Open'}
                                </Button>
                              )}
