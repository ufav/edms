                  ...
                  <Box sx={{ mb: 3 }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                      <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap', width: '100%' }}>
                        ...
                        <FormControl size="small" variant="standard" sx={{ width: '25%' }}>
                          ...
                          <Select
                            ...
                            onChange={(e) => {
                              ...
                              if (!value) return;
                              const selectedSeq = getAvailableSequences.find(seq => seq.revision_description_id === Number(value));
                              if (selectedSeq) {
                                setNewRule(prev => ({
                                  ...prev,
                        ...
                        <FormControl size="small" variant="standard" sx={{ minWidth: 100 }}>
                          ...
                          <Select
                            ...
                            onChange={(e) => setNewRule(prev => ({ ...prev, operator: e.target.value as 'equals' | 'not_equals' }))}
                        ...
                        <Autocomplete
                          ...
                          onChange={(_, newValue) => setNewRule(prev => ({ ...prev, review_code_ids: (newValue as any[]).map(rc => (rc as any).id) }))}
                          renderInput={(params) => (
                            <TextField {...params} variant="standard" label={t('workflows.fields.review_codes')} />
                          )}
                        />
                      </Box>
                        ...
                        <FormControl size="small" variant="standard" sx={{ width: '25%' }}>
                          ...
                          <Select
                            ...
                            onChange={(e) => {
                              const value = e.target.value;
                              if (!value) {
                                setNewRule(prev => ({ ...prev, next_revision_description_id: null, next_revision_step_id: null }));
                              } else {
                                const selectedSeq = getAvailableSequences.find(seq => seq.revision_description_id === Number(value));
                                if (selectedSeq) {
                                  setNewRule(prev => ({ ...prev, next_revision_description_id: selectedSeq.revision_description_id, next_revision_step_id: selectedSeq.revision_step_id }));
                                }
                              }
                            }}
                            label={t('workflows.fields.sequence')}
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                        <Button variant="contained" size="small" onClick={addRule} disabled={!newRule.current_revision_description_id || newRule.review_code_ids.length === 0}>
                          {t('workflows.actions.add')}
                        </Button>
                      </Box>
                    </Box>
                      ...
                      {workflowRules.map((rule, index) => (
                        <Box key={index} sx={{ p: 2, mb: 1 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                            <Typography variant="body2" sx={{ minWidth: '20px' }}>{index + 1}.</Typography>
                            <Typography variant="body2" color="text.secondary">{t('workflows.conditions.if')}</Typography>
                            <Chip label={rule.current_revision_description?.code || workflowStore.revisionDescriptions.find(rd => rd.id === rule.current_revision_description_id)?.code || 'U'} color="primary" size="small" />
                            <Typography variant="body2">-</Typography>
                            <Chip label={rule.current_revision_step?.code || workflowStore.revisionSteps.find(rs => rs.id === rule.current_revision_step_id)?.code || 'TCO'} color="secondary" size="small" />
                            <Typography variant="body2" color="text.secondary">{rule.operator === 'equals' ? t('workflows.operators.equals') : t('workflows.operators.not_equals')}</Typography>
                            {(rule.review_codes || []).map((rc: any, idx: number) => (<Chip key={idx} label={rc.code} color="warning" size="small" />))}
                            <Typography variant="body2" color="text.secondary">→</Typography>
                            {rule.next_revision_description_id ? (
                              <>
                                <Chip label={rule.next_revision_description?.code || workflowStore.revisionDescriptions.find(rd => rd.id === rule.next_revision_description_id)?.code || 'U'} color="success" size="small" />
                                <Typography variant="body2">-</Typography>
                                <Chip label={rule.next_revision_step?.code || workflowStore.revisionSteps.find(rs => rs.id === rule.next_revision_step_id)?.code || 'Construction'} color="success" size="small" />
                              </>
                            ) : (
                              <Chip label={`+1 ${t('workflows.messages.increment_number_short')}`} color="default" size="small" />
                            )}
                            <Box sx={{ flexGrow: 1 }} />
